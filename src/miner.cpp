...
// Copyright (c) 2013 The MyBitcoinClassic developers
...
bool FillMap(CWallet *pwallet, uint32_t nUpperTime, MidstateMap &inputsMap)
{
    ...
    CTxDB txdb("r");
    {
        ...
        CoinsSet setCoins;
        int64_t nValueIn = 0;
        if (!pwallet->SelectCoinsSimple(nBalance - nReserveBalance, MIN_TX_FEE, MAX_SEND, nUpperTime, nCoinbaseMaturity + 10, setCoins, nValueIn))
...
void ThreadStakeMiner(void* parg)
{
    ...
    RenameThread("MyBitcoinClassic-miner");
