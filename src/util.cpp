...
#else
    const char* pszModule = "MyBitcoinClassic";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MyBitcoinClassic
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MyBitcoinClassic
    // Mac: ~/Library/Application Support/MyBitcoinClassic
    // Unix: ~/.MyBitcoinClassic
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MyBitcoinClassic";
#else
    fs::path pathRet;
    ...
    return pathRet / "MyBitcoinClassic";
#else
    // Unix
    return pathRet / ".MyBitcoinClassic";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MyBitcoinClassic.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "MyBitcoinClassicd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong MyBitcoinClassic will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("MyBitcoinClassic"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
