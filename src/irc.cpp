...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("MyBitcoinClassic-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MyBitcoinClassicTEST2\r");
            Send(hSocket, "WHO #MyBitcoinClassicTEST2\r");
        } else {
            // randomly join #MyBitcoinClassic00-#MyBitcoinClassic05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #MyBitcoinClassic%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MyBitcoinClassic%02d\r", channel_number).c_str());
        }
