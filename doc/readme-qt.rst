MyBitcoinClassic-qt: Qt4 GUI for MyBitcoinClassic
===============================
...
Alternatively, install Qt Creator and open the `MyBitcoinClassic-qt.pro` file.

An executable named `MyBitcoinClassic-qt` will be built.
...
A warning for people using the *static binary* version of MyBitcoinClassic on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of MyBitcoinClassic is linked against libdb5.3.
...
installed causes MyBitcoinClassic-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
